    ...
    if ($connexion) {
        ...
        if (isset($_GET['id'])) {
            ...
            try {
                ...
                $query_plage = "DELETE FROM plage WHERE ID_PLAGE = :id_plage";
                $stmt_plage = $connexion->prepare($query_plage);
                $stmt_plage->bindParam(':id_plage', $id_plage);
                $stmt_plage->execute();

                // Rediriger vers la page de liste des villes après la suppression réussie
                header('Location:./listePlage.php?suppression=1');
                exit(); // Terminer le script après la redirection
            } catch (PDOException $e) {
                echo "Erreur lors de la suppression de la plage : " . $e->getMessage();
            }
        } else {
            echo "ID de plage non fourni pour la suppression.";
        }
    } else {
